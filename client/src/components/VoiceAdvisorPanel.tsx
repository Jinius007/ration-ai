...
export function VoiceAdvisorPanel({ autoStart = false }: { autoStart?: boolean }) {
  ...
  const startCall = useCallback(async () => {
    if (!speechSupported()) {
      setError(
        lang === "en"
          ? "Use Chrome or Edge on phone/desktop for voice. You can type answers below."
    ...
    <div className="flex flex-col flex-1 min-h-0 px-4 py-4 sm:px-6 sm:py-6 max-w-2xl mx-auto w-full gap-4">
      <div className="flex flex-col items-center gap-3 py-2">
        ...
        <div className="text-center">
          ...
          <p className="text-[10px] text-[var(--accent-light)] mt-1">
            {lang === "en" ? "Free · Browser voice · No API key" : "Muft · Browser awaz · Koi API key nahi"}
          </p>
        </div>
        ...
        {!live ? (
          ...
          <>
            <button
              ...
              title={lang === "en" ? "Type answer" : "Likhit jawab"}
            >
              <Keyboard size={20} />
            </button>
