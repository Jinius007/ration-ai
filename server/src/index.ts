...
/** Signed URL + public config for browser voice session (API key stays server-side) */
...
app.get("/api/elevenlabs/signed-url", async (_req, res) => {
  if (!ELEVENLABS_API_KEY || !ELEVENLABS_AGENT_ID) {
    res.status(503).json({
      error: "Set ELEVENLABS_API_KEY and ELEVENLABS_AGENT_ID in server/.env",
    });
    return;
  }
  try {
...
/** Simplified body for ElevenLabs agent / voice client tools */
...
/** ElevenLabs server tool webhook — optional if using public URL */
